...
Далі у фоні: завантаження → витяг тексту (скан без тексту — окремий статус) → фрагменти → ембединги
OpenAI `text-embedding-3-large` → структура «розділ → тема → сторінки» (Claude Opus 5.5, роль
`indexing_structure`). Кожен ШІ-виклик іде через маршрутизатор (таблиця `model_routes`) і записується в
`ai_calls` з вартістю; місячна сума — у `spend_months`. У режимі бюджету нові книги відкладаються.
Фонова робота запускається одразу після дії тата (`after()`), продовжується, поки відкрито «Мої книги»,
і раз на добу — Vercel Cron (`/api/jobs/tick?sync=1`, захищено `CRON_SECRET`).
