...
describe("mergeVerdict", () => {
  it("uses layer 2 directly when there is no escalation", () => {
    ...
    expect(mergeVerdict(false, v, null)).toEqual({ ...v, layer1Flagged: false, escalated: false });
  });
  it("the escalation verdict wins over layer 2's own guess", () => {
    ...
    expect(mergeVerdict(true, layer2, escalation)).toEqual({ ...escalation, layer1Flagged: true, escalated: true });
  });
});
