...
| Поле | Значення |
|---|---|
| Версія | 0.3 (на затвердження, Фаза 1; оновлено 2026-09-25 за стратегічними рішеннями Алекса: старт «Економ», модульність — ADR-017, готовність до комерціалізації — ADR-018; **0.3 — образ репетитора: ім'я й голос обирає дитина, аватар-картинка з модерацією, анімація за станами голосу — розд. 7.6, ADR-006, ADR-019**) |
| Дата | 2026-09-25 |
| Автор | `architect` |
| Вхідні | `docs/00-product-brief.md`, `docs/01-requirements.md` v0.3 (затв. 2026-09-25), `docs/05-backlog.md` v0.3 |
| Пов'язані | `docs/03-resources-and-costs.md` (акаунти, ключі, кошторис), `docs/adr/001…019` |
...
**Образ репетитора обирає донька** (рішення 2026-09-25): ім'я (з підказок або своє, з автоматичною перевіркою), голос — жіночий або чоловічий з кількох українських голосів, які ви попередньо прослухаєте й затвердите, і аватар — готовий або своя картинка. Картинку перевіряє спеціальний безкоштовний «фільтр недоречного вмісту» (вона не потрапляє до ШІ-співрозмовників), ви бачите її в кабінеті й можете замінити. Аватар «оживає» простою анімацією (слухаю / думаю / говорю) — $0. Зміна голосу: нові уроки одразу звучать новим голосом, старі переозвучуються **лише коли донька їх відкриває** — у «Економі» ≈ **$0–9 за місяць після зміни** (розд. 7.6). «Говорящий» аватар з рухом губ — Фаза 2, окреме рішення про вартість.
...
| Аватар репетитора (MVP) | Картинка (готова або завантажена) у приватному бакеті Supabase Storage; **анімація CSS/Canvas** на клієнті за станами голосу; перевірка: обличчя — **на пристрої** (MediaPipe Face Detector), недоречний вміст — **OpenAI `omni-moderation`** (безкоштовно), резерв — **Google Cloud Vision SafeSearch** (1 000 зображень/міс безкоштовно) (ADR-019) | $0/міс; зображення не йде в розмовні моделі (US-1.8, ВП-33) | Модерація через vision-LLM (Haiku/Gemini Flash, ≈ $0,001/зобр.) — дешево, але це «розмовна» модель, що суперечить US-1.8; говорящий аватар — Фаза 2 |
...
flowchart LR
  subgraph DEV["Пристрої: планшет доньки, телефон/ноутбук тата (PWA, Chrome)"]
    ...
    FD["Перевірка обличчя на пристрої<br/>(MediaPipe) + стиснення аватара"]
  end
  ...
  FD --> API
  API -- "аватар: omni-moderation / Vision SafeSearch (резерв)" --> LLM
...
| `avatar_moderation` (не LLM) | стиснене зображення аватара без EXIF → **лише** спеціалізований класифікатор (`omni-moderation` / Vision SafeSearch) | **розмовним моделям зображення не передається ніколи** (US-1.8, NFR-PRIV-12) |
...
#### 7.6.4. Аватар: зберігання, обмеження, перевірка, заміна татом (ADR-019)
...
| Перевірка, шар 0 (на пристрої, $0) | MediaPipe Face Detector (у браузері, зображення нікуди не йде): знайдено реалістичне обличчя → підказка «Краще не використовувати фото реальних людей» (US-1.8); якщо дитина все ж завантажує — статус `pending_parent` |
| Перевірка, шар 1 (сервер, $0) | **OpenAI `omni-moderation-latest`** з зображенням — спеціалізований класифікатор модерації, не розмовна модель; безкоштовний; дані API не використовуються для навчання |
| Резерв | **Google Cloud Vision SafeSearch** (перші 1 000 зображень/міс безкоштовно, далі $1,50 / 1 000) — інший провайдер; обидва недоступні → `pending_parent` |
| Результат | Чисто → `approved`, діє одразу. Сумнів/обличчя → `pending_parent`: дитина бачить попередній аватар і «Покажемо татові», тато затверджує/відхиляє. Недоречне → `rejected`, м'яке повідомлення дитині, подія тату з мініатюрою (файл у карантині, видно лише татові, автовидалення через 30 днів). Категорії самоушкодження / сексуального вмісту → `safety_events` з severity `urgent` (розд. 9.5) |
| Заміна татом | Кабінет → «Образ репетитора»: мініатюра поточного аватара й історія; «Замінити» (готовий або свій файл — ті самі обмеження; модерація виконується, але затверджується автоматично), «Скинути на вогник»; перемикач «Дитина може змінювати аватар / голос / ім'я» (`persona_child_editable`) |
| Розмовні моделі | Зображення аватара **не передається** жодній розмовній чи генеративній моделі (NFR-PRIV-12) |
| Вартість | ≈ **$0/міс** (модерація безкоштовна; сховище — кілобайти) |
...
| `tutor_avatars` | `kind` (`preset`/`image`; Фаза 2 — `rig`/`vendor_avatar`), `preset_key`, `storage_path`, `bytes`, `sha256`, `width`, `height`, `uploaded_by` (`child`/`parent`), `moderation_status` (`pending`/`approved`/`pending_parent`/`rejected`), `moderation_result jsonb` (провайдер, категорії, оцінки, «обличчя»), `reviewed_by`, `reviewed_at`, `created_at`, `deleted_at` | Бакет `tutor-avatars` (розд. 7.6.4); `rejected` — у карантині 30 днів. |
...
- **Дитина:** читання бібліотеки, своїх сесій, чатів, карти знань (для прогресу), балів, `tutor_voices` (`active`), власних `tutor_avatars` (крім `rejected`); зміна образу — лише через серверний маршрут `persona` (перевірки й модерація); **жодного доступу** до `parent_settings`, `model_routes`, `spend_months`, `ai_calls`, `safety_events`, `notifications`, `reports`.
...
| Аватар репетитора | Приватний бакет `tutor-avatars` з `family_id` у шляху; стиснення й зняття EXIF на пристрої + повторне перекодування на сервері; заборона SVG/GIF; перевірка обличчя на пристрої, модерація спеціалізованим класифікатором (не розмовною моделлю); `pending_parent` / карантин; тато замінює (розд. 7.6.4, ADR-019) | NFR-SAFE-14, NFR-PRIV-5, 12 |
...
| AR-R17 | Модерація зображень пропускає недоречне або помилково блокує дитячий малюнок; `omni-moderation` оцінює лише частину категорій для зображень | Безпека / UX | Шар обличчя на пристрої, резерв Vision SafeSearch, `pending_parent` при сумніві, тато бачить кожен новий аватар і може замінити |
