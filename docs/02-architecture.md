...
| Поле | Значення |
|---|---|
| Версія | 0.3 (на затвердження, Фаза 1; оновлено 2026-09-25 за стратегічними рішеннями Алекса: старт «Економ», модульність — ADR-017, готовність до комерціалізації — ADR-018; **0.3 — образ репетитора: ім'я й голос обирає дитина, аватар-картинка з модерацією, анімація за станами голосу — розд. 7.6, ADR-006, ADR-019**; **0.3.1 (2026-09-25) — модерацію аватара прибрано з MVP (ADR-019, ADR-018); інтерактивні компоненти уроку — розд. 14.3, ADR-020**; **0.3.2 (2026-09-25) — інтерактив у MVP прийнято (ADR-020); багаторічність 6→11 клас — розд. 8, 14.4, ADR-021; точка розширення для відеоуроків — розд. 14, ADR-016**; **0.3.3 (2026-09-25, рішення PO D-55) — педагогічний конвеєр генерації уроку (планування → сценарій → незалежна рецензія іншим провайдером → доопрацювання → «методичний паспорт»); книги з Drive — лише база програми, ШІ творить урок за доказовими методиками — розд. 7.7, 8.2, 9.2, ADR-022; примітки в ADR-005, ADR-014**; **0.3.4 (2026-09-26, доповнення PO) — план і рецензія враховують «гачок», видимий прогрес і теплоту тону (розд. 7.7); переоцінка живого голосу (ElevenLabs vs Claude/OpenAI Realtime/Gemini Live) — розд. 7.4, ADR-006 (статус: пропозиція, рішення за PO)**; **0.4 (2026-09-26, рішення PO D-76/D-77) — фонове «прогрівання» бібліотеки уроків за сигналом D-65 (`is_current`), пріоритизація черги, обмеження паралельності й бюджету, де-дуплікація — розд. 7.7a, ADR-023; наскрізний поділ системи на два бюджети швидкості («повільний, одноразовий» Шар 1 для бібліотеки vs «швидкий, живий» Шар 2 для будь-якої інтерактивної відповіді дитині, зокрема майбутнього US-6.15/S27) — розд. 7.7b, новий рядок `step_reinforcement` у розд. 6.1/7.3, ADR-023; відкрите питання PO — компроміс «≈60–90 с з чесним прогресом» vs «швидка нерецензована чернетка» для рідкісного залишкового «холодного» випадку**; **0.5 (2026-09-26, питання PO вночі) — завантаження книг з комп'ютера в «Мої книги»: спільний OAuth `drive.file`-конектор для архіву (ADR-013/S17) і нової app-owned папки завантажень, автоshare сервісному акаунту, стрімінговий upload з практичним лімітом ≈50 МБ (нижче `MAX_FILE_BYTES=150 МБ`), пряме потрапляння в наявний конвеєр S1/S1b без дублювання в Supabase Storage — розд. 10.3, ADR-024**) |
| Дата | 2026-09-26 |
| Автор | `architect` |
...
| Пов'язані | `docs/03-resources-and-costs.md` (акаунти, ключі, кошторис), `docs/adr/001…023` |
...
**Модульність і «на виріст».** Нові предмети й будь-які книги з Drive (не лише підручники) додаються **даними**, без програміста; нові навчальні модулі (напр., «Додаткове навчання: програмування», підготовка до іспитів) — підключаються до готового ядра, не переробляючи його (розд. 14.1, ADR-017). У базі одразу закладено поняття «сім'я», щоб за бажання колись відкрити продукт іншим сім'ям без переписування (розд. 14.2, ADR-018). Обидва рішення — $0 на місяць. Застосунок розрахований на **7 років (6→11 клас)**: теми й підручники знають свій клас, уроки й чати — свій навчальний рік, карта знань переходить далі, щомісяця повна копія даних лягає на ваш Drive (розд. 14.4, ADR-021; ≈ 1,5 дня розробки, $0/міс). Інтерактивні вправи (числова пряма, перетягування карток / «шкала часу», покрокові пояснення) — **у MVP** (ваше рішення, ≈ 4–5 днів).
...
| Google Drive | **Сервісний акаунт** (читання папки матеріалів) + **OAuth `drive.file` тата** (один конектор для запису архіву **і** нової папки книг, завантажених з кабінету) | Найпростіший надійний варіант з урахуванням обмежень Google (розд. 10, ADR-003, ADR-024). | Лише OAuth у режимі Testing — токени зникають кожні 7 днів. |
...
| `drive` | Список і завантаження файлів папки матеріалів; перевірка публічності; запис архіву; **завантаження книги з браузера в app-owned папку «Мої книги» + автоshare сервісному акаунту (ADR-024)** | US-2.1, 2.7 КП-5, 18.1, NFR-PRIV-7 |
...
| `jobs` | Таблиця задач, `tick`-обробник, повтори, ідемпотентність, **фонове прогрівання бібліотеки** (`library.warm_topic`, дедуплікація, пріоритет, ліміти паралельності/бюджету) | ADR-015, ADR-023 |
...
> **Правило двох бюджетів швидкості (D-77, розд. 7.7b).** Ролі, що виконуються в межах живого запиту дитини (`answer_evaluation`, `step_reinforcement`, `tutor_chat`, `friend_chat`, `voice_agent`, `safety_moderator`) — **завжди один виклик `callModel`**, без ланцюжка «згенерувати → відрецензувати іншим провайдером синхронно». Ролі важкого одноразового конвеєра (`lesson_planning`, `lesson_generation`, `lesson_review`) викликаються **лише** з фонових задач (`jobs`: прогрів 7.7a або «холодний» старт сесії, і ніколи — з обробника відповіді на крок чи повідомлення чату).
...
|---|---|---|---|---|---|
| `lesson_planning` — **методичний план блоку** (мета, міскончепції, доказові прийоми, структура) — розд. 7.7, ADR-022 | **Claude Opus 5.5** | GPT-5.6 Sol | — | Gemini 3.8 Flash (план спрощується) | Найкращий провайдер для методичного мислення й дотримання інструкцій; той самий провайдер, що й генерація — план і сценарій узгоджені термінологією |
...
| `lesson_review` — **незалежна рецензія блоку за рубрикою** (методика, фактична відповідність підручнику, вік/клас, різноманітність, безпека, ПД) — розд. 7.7, ADR-022 | **GPT-5.6 Sol** (OpenAI) | Gemini 3.1 Pro | Claude Opus 5.5 (лише якщо і основний, і резерв недоступні — тоді рецензент і автор різних провайдерів все одно дотримано, бо `lesson_generation`/`lesson_planning` — Claude) | Gemini 3.8 Flash (спрощена рубрика, лише безпека+фактчек) | **Обов'язково інший провайдер, ніж `lesson_generation`** (незалежність рецензії, як і резерв ролей уроку, US-13.1 КП-2); GPT-5.6 Sol — та сама модель, що вже є резервом генерації, отже якість підтверджена й окремого спайку не потрібно |
...
| `step_reinforcement` — **новий** (D-74/D-77, розд. 7.7b): пояснення помилки + ще одна вправа на закріплення в межах кроку; **шар 2 («живий»)** — один виклик, **ніколи** не веде до `lesson_planning`/`lesson_review` синхронно | **Claude Sonnet 5** | GPT-5.6 Terra | Claude Opus 5.5 (складніший випадок, той самий одиничний виклик) | Gemini 3.8 Flash | Мета — латентність рівня `answer_evaluation` (≤ 5–8 с); якість довгострокового перевикористання зберігається окремо — фонова задача `library.harvest_reinforcement` асинхронно відбирає вдалі приклади й пропускає їх через повний конвеєр (7.7b), не в шляху дитини |
...
**Переоцінка 2026-09-26 (доповнення PO до D-55): «чому не голос Claude/OpenAI напряму?»** Перевірено: голосовий режим **Claude** — функція застосунку Claude для передплатників (не API-продукт) і сам усередині працює через ElevenLabs TTS, не нативну мовну модель — тобто прямого шляху «підписка Claude → голос дитини» не існує. **OpenAI Realtime API** (`gpt-realtime-2`, нативний speech-to-speech) і **Gemini Live API** (нативне аудіо) — окремі платні API-продукти, дешевші за ElevenLabs (≈ $0,05/хв і ≈ $0,037/хв відповідно), але якість **української** офіційно не підтверджена на рівні «топових» мов і не перевірена на дитячому мовленні (як і раніше — AR-R11). **Рекомендація:** основний голос репетитора лишити на ElevenLabs (єдиний варіант із підтвердженою якістю української й готовим каталогом голосів під вибір дитини); спайк S13 розширити коротким тестом Gemini Live / gpt-realtime-2-mini на тих самих записах доньки — якщо якість підтвердиться, розглянути гібрид (дешевший підтверджений варіант — для ролі «ШІ-друг», `VoiceProvider`, D-4). Статус ADR-006 змінено на «пропозиція — рішення за PO»; деталі, порівняльні таблиці й джерела — ADR-006, `docs/03` розд. 6.
...
- **Граматичний рід** (`tutor_gender`: `f`/`m`) береться з обраного голосу, а не з імені: агент і тексти інтерфейсу узгоджують «я зрозуміла / зрозумів», «Оля каже / Сашко каже».
  **Поки голос не обрано** (`child_profile.tutor_voice_id is null`: S0–S11 або дитина лишила голос за замовчуванням) рід береться з вибору імені й зберігається в `child_profile.tutor_name_gender` (`f` за замовчуванням): для імені зі списку — група (`tutor_name_options.f`/`.m`), для власного імені — явний вибір дитини «Вона / Він» на тому ж кроці. Щойно голос обрано — діє рід голосу; міграція даних на S12 не потрібна (BUG-002). Усі тексти з родом — через один хелпер `gendered()` (`app/src/lib/persona/gender.ts`).
...
### 7.7. Педагогічний конвеєр генерації уроку (рішення PO D-55, 2026-09-25; ADR-022)

> **Простими словами.** Підручник з Drive — це лише «база програми»: звідки береться тема, факти й термінологія. Сам урок ШІ не переказує підручник сухо, а **проєктує** за перевіреними прийомами навчання (мета уроку, типові помилки дітей у цій темі, вправи на пригадування, приклади з поясненнями, перевірочні питання). Щоб довіряти якості без ручної перевірки кожного уроку, **інша ШІ-модель (інший постачальник)** незалежно перевіряє готовий урок за чіткою рубрикою — і лише після її схвалення урок потрапляє в бібліотеку й показується дитині. Кожен урок має «методичний паспорт» — коротку картку, яку бачить тато: мета, прийоми, оцінка рецензента.

**Конвеєр (заміняє прямий виклик `lesson_generation` у 9.2; спрацьовує один раз на блок — бібліотека, ADR-014):**

```mermaid
flowchart LR
  A[Немає блоку в бібліотеці] --> P["1. lesson_planning (Claude Opus 5.5)<br/>мета · міскончепції · доказові прийоми · структура кроків"]
  P --> G["2. lesson_generation (Claude Opus 5.5)<br/>сценарій кроків за планом + фрагменти підручника"]
  G --> R{"3. lesson_review (GPT-5.6 Sol)<br/>рубрика: методика/факти/вік/різноманітність/безпека/ПД"}
  R -->|approved| S["Збереження: library_items status=active<br/>+ методичний паспорт (pedagogy jsonb)"]
  R -->|revise, ≤ 2 ітерації| G
  R -->|rejected після 2 ітерацій| M["status=needs_review<br/>подія тату «потрібна ручна перевірка»<br/>дитині НЕ показується"]
  S --> V["Показ дитині дозволено"]
```

> **Доповнення PO (2026-09-26, до D-55): «не вирішувати задачу в лоб».** Мета конвеєра — не лише коректний і безпечний урок, а **захопливий**: діти вчаться, коли цікаво, коли видно результат і коли є довірливий контакт з «учителем». Тому методичний план і рецензія явно враховують не лише коректність, а **інтерес і теплоту**.

1. **Методичне планування** (`lesson_planning`). Вхід: тема, фрагменти підручника (факти й термінологія — «база програми», рішення D-55), клас/НУШ, тривалість блоку, стислий поведінковий профіль дитини й тег інтересу (без ПД, як у `lesson_generation` — розд. 5.4), реєстр доступних інтерактивних компонентів (ADR-020). Вихід (JSON): `learning_goal`, `misconceptions[]` (типові помилки теми — з педагогічних знань моделі, не з підручника), `techniques[]` (обрані доказові прийоми: retrieval practice, worked example, interleaving, formative check тощо — з коротким обґрунтуванням «чому тут»), `hook` (короткий «гачок» на старті блоку — цікаве питання/факт/міні-задача під тег інтересу дитини), `progress_markers[]` (де в блоці дитина побачить видимий результат: «зібрав», «розв'язав», прогрес-індикатор), `tone_notes` (як репетитор звертається до попередніх успіхів теми й заохочує зусилля, не оцінку — коротка інструкція для кроку `voice_dialog`/`tutor_chat`), `step_plan[]` (типи кроків, їх призначення й де доречний обраний інтерактивний/візуальний компонент з реєстру ADR-020 для естетики й «вау-моменту»). Це **план**, не текст уроку — дешевший виклик, який задає і методичну, і емоційну рамку.
2. **Генерація сценарію** (`lesson_generation`, без змін у форматі — ADR-014, ADR-020) — той самий Opus 5.5, отримує план (зокрема `hook`, `progress_markers`, `tone_notes`) + фрагменти підручника з посиланнями на сторінки (кожен факт — `source_refs`, як і зараз) → повний набір `library_steps`, перший крок блоку — реалізація `hook`.
3. **Незалежна рецензія** (`lesson_review`) — **обов'язково інший провайдер** (GPT-5.6 Sol), той самий принцип, що й для резерву ролей уроку (US-13.1 КП-2, ADR-005). Отримує сценарій + методичний план + фрагменти підручника (для перевірки фактів) і оцінює за фіксованою рубрикою (structured output, 0–2 бали за критерій):
   - **методика** — чи відповідають кроки заявленим прийомам, чи є пригадування/перевірка розуміння, а не лише виклад;
   - **фактична відповідність підручнику** — кожен факт має `source_refs`, що підтверджує його на вказаній сторінці; термінологія збігається з підручником;
   - **вік/клас** — складність мови й прикладів відповідає класу;
   - **зацікавленість** — чи є справжній «гачок» на старті й хоч один «вау-момент»/видимий результат у блоці, а не сухий виклад (PO 2026-09-26);
   - **тепло й довіра тону** — репетитор заохочує зусилля, не засуджує помилки, звертання доброзичливе (звіряється з преамбулою безпеки, розд. 5.4);
   - **різноманітність** — блок не повторює патерн (типи кроків, приклади, «гачок») останніх N уроків теми/предмета (перевіряється порівнянням з попередніми `library_items` тієї ж теми/предмета);
   - **безпека** — немає небезпечного, тривожного чи недоречного контенту (та сама преамбула безпеки, розд. 5.4);
   - **відсутність даних дитини** — немає нікнейму, імен, персональних деталей (лише плейсхолдер `{{nickname}}`).
   Вердикт: `approved` (усі критерії ≥ порогу) / `revise` (конкретні зауваження по кроках) / `rejected` (грубе порушення безпеки чи фактів).
4. **Доопрацювання.** `revise` → `lesson_generation` отримує зауваження рецензента як частину промпту → нова спроба → знову на рецензію. **Максимум 2 ітерації**; якщо після них не `approved` — блок зберігається зі статусом `needs_review`, **дитині не показується**, тату — подія в центрі сповіщень з посиланням на паспорт і зауваження (ручна перевірка/перегенерація тата, як «Перегенерувати», ADR-014).
5. **Публікація.** Лише `status=active` (тобто пройшов рецензію) бере оркестратор при плануванні уроку (9.2) — правило перевіряється на рівні запиту бібліотеки, а не лише UI (обхід неможливий).
6. **Роль Gemini в конвеєрі.** Окремого генеративного «візуального» етапу в MVP **немає**: інтерактивні компоненти й так виключають довільну графіку (ADR-020, каталог ресурсів), тому Gemini як «третій постачальник» тут — лише **резерв рецензії** (`lesson_review` fallback) і залишається основним для вже наявних мультимодальних ролей (`vision_notebook`, `vision_screenshot`, розд. 7.5). Додавати Gemini-генерацію зображень/діаграм зараз — передчасне ускладнення без підтвердженої користі; переглянути в R2, якщо з'явиться конкретна потреба (напр., власні SVG-карти для компонента `map`).
7. **Режим бюджету.** У `budget`-стані рецензія **не вимикається** (безпека й якість — не економляться), але переходить на `economy_model` рецензента (Gemini 3.8 Flash, спрощена рубрика: лише факти + безпека + вік); генерація нових блоків у `budget` і так рідкісна (більшість — повтори з бібліотеки).

**Методичний паспорт** (`library_items.pedagogy jsonb`) — те, що бачить тато (US-13.4, розд. 8.2):
```json
{
  "learning_goal": "...",
  "misconceptions": ["...", "..."],
  "techniques": [{"name": "retrieval_practice", "why": "..."}],
  "hook": "...",
  "progress_markers": ["..."],
  "tone_notes": "...",
  "review": {
    "model": "gpt-5.6-sol", "verdict": "approved",
    "scores": {"pedagogy": 2, "factual": 2, "age_fit": 2, "engagement": 2, "tone": 2, "diversity": 1, "safety": 2, "no_pii": 2},
    "notes": "...", "iterations": 1
  }
}
```
У кабінеті тата — компактна картка «Методичний паспорт» на екрані уроку/бібліотеки: мета, 2–3 прийоми, «гачок» уроку одним реченням, підсумковий вердикт і оцінка рецензента по всіх критеріях **включно з «Цікаво» й «Тепло тону»** (без сирого JSON); кнопка «Перегенерувати» лишається (ADR-014). Оцінка «цікаво / корисно» від дитини (проста реакція після блоку, US-19.4) зберігається окремо (`library_items.child_feedback`) і **доповнює**, а не замінює, паспорт — так тато бачить і формальну оцінку рецензента, і живий відгук дитини поруч.

**Що зараз, що пізніше.** У MVP — весь конвеєр як описано (планування → генерація → рецензія → ≤ 2 доопрацювання → паспорт); це мінімум, потрібний, щоб виконати рішення D-55 «ШІ творить за методиками, а не переказує підручник» без ручної перевірки кожного уроку. **Свідомо відкладено:** окремий генеративний візуальний етап (Gemini-зображення/діаграми) — лише за конкретною потребою в R2; A/B-порівняння кількох варіантів сценарію перед рецензією (дорожче, поки не обґрунтовано); авто-навчання рубрики на відгуках тата — R3.

### 7.7a. Фонове «прогрівання» бібліотеки — план елевації D-66 → D-76 (ADR-023)

> **Простими словами.** Той самий конвеєр із 7.7 (план → сценарій → рецензія) триває майже хвилину. Поки він виконувався лише в момент, коли дитина вперше відкривала тему («на холодну») — звідси довге «Готуємо урок…», яке PO порівняв (не на користь) зі швидкістю прямих відповідей ChatGPT/Claude (D-76). Рішення — **той самий** конвеєр, викликаний **раніше й у фоні**, через уже наявну чергу `jobs` (ADR-015), а не окрема логіка. Мета — щоб дитина в переважній більшості випадків відкривала вже готову тему; випадок «на холодну» лишається (мінімум D-66), але стає рідкісним винятком.

**1. Коли запускати (тригер).** Лише коли **тато позначає тему(и) «поточними»** (D-65: `topics.is_current = true`, одна, кілька або всі теми предмета одночасно) — **не** одразу після успішної індексації підручника. Обґрунтування вартість/швидкість:
- Індексований підручник 6 класу — це, як правило, кілька десятків тем на весь навчальний рік; переважна більшість буде відкрита дитиною лише через тижні чи місяці (або взагалі не в цьому семестрі). Прогрівати все одразу після індексації означає **платити наперед за контент, який може бути ніколи не пройдено** — пряме порушення A-P2 (економія через перевикористання, не через передчасні витрати) і ризик, на який прямо вказав PO.
- Позначення «поточна» (D-65) — це вже явний сигнал наміру тата: «ці теми дитина реально може обрати найближчим часом». Прогрів саме на цей сигнал прив'язує витрати до **ймовірності реального використання**, а не до факту існування теми в підручнику.
- Тато й так позначає теми поточними як частину звичайного робочого процесу (US-3.1), тож додаткової дії від нього не потрібно — прогрів непомітний, без нового кроку в UI.

**2. Що саме прогріває один «warm»-job.** Job типу `library.warm_topic` (payload: `topic_id`, `family_id`, `module_code`, `academic_year_id`) виконує **той самий програмний шлях**, що й «холодний» старт уроку в 9.2 (крок «немає активного блоку → `lesson_planning` → `lesson_generation` → `lesson_review` → …») — одна функція `ensureActiveLibraryBlock(topicId)`, яку викликають **обидва** шляхи (і кабінет тата через job, і оркестратор при вході дитини). Це не нова логіка — лише нова точка виклику; дублювання конвеєра неприпустиме (A-P3).
- Прогрівається **лише перший блок теми** (той, що потрібен синхронно на старті сесії, розд. 9.2), а не всі блоки повного 30/45-хв уроку одразу. Наступні блоки сесії (2-й, 3-й, 4-й…) генеруються, як і зараз, **прогресивно під час живої сесії** (з тим самим запасом часу, що вже приховує затримку кроків усередині блоку, розд. 9.2) — так вартість прогріву однієї теми ≈ вартість **одного блоку** конвеєра (≈ $0,27–0,33, розд. 3.2а), а не всього уроку (≈ $1,3+). Це ключовий важіль вартість/швидкість: прогрів знімає **саме ту** затримку, на яку скаржився PO (перший екран уроку), і коштує в ≈ 4 рази менше за прогрів усього уроку наперед.

**3. Пріоритизація черги (кілька тем позначено одночасно, D-65).** У наявній схемі `is_current` — це вже й є позначка «Пріоритет» (UI-бейдж, `priorityBadge`), тому серед тем, позначених одним пакетним рішенням тата, усі формально рівно «пріоритетні» — окремого другого рівня пріоритету в даних немає. Черга обробляється в порядку:
1. `topics.sort_order` (порядок теми в програмі підручника) **зростаюче** — раніша за програмою тема — це природний наступний вибір дитини (яка зазвичай іде послідовно), тому прогріваємо її першою;
2. за рівних `sort_order` — порядок, у якому тато позначив теми (`created_at`/`updated_at` події `is_current`), тобто просте FIFO.
Нового поля в БД для цього не потрібно — сортування виконує сам обробник черги (`ORDER BY topics.sort_order, jobs.created_at`) при виборі задач у `tick`. **Відкрите питання до PO** (якщо захоче окремо виділяти «найважливішу» серед кількох поточних тем — окрема позначка понад `is_current` — не в MVP цього фіксу).

**4. Обмеження паралельності й бюджету.**
- **Глобальний ліміт одночасних warm-конвеєрів** — `LIBRARY_WARM_MAX_CONCURRENT` (за замовчуванням **2**): обробник `tick` рахує задачі `library.warm_topic` зі статусом `running` і бере в роботу не більше `LIBRARY_WARM_MAX_CONCURRENT - running` нових (`FOR UPDATE SKIP LOCKED LIMIT …`, той самий патерн, що й для інших типів `jobs`, ADR-015). Це навмисно **обережне** число: кожен конвеєр сам по собі — це 2–3 послідовні виклики різних провайдерів (Claude, потім OpenAI), тож 2 паралельні прогріви = до 4–6 одночасних викликів моделей — з запасом нижче типових лімітів запитів/хвилину для стартового (низького) рівня оплати Anthropic/OpenAI при $50/$20 передоплати (розд. 1.7–1.8 `docs/03`); **розробник звіряє фактичний RPM-ліміт акаунта на S1/S27 і за потреби зменшує число до 1**.
- **Не перетинати бюджетні пороги непомітно для тата.** Прогрів — це виклики через звичайний `callModel` (розд. 6.2), тож існуючі пороги 80/100/110 % (`spend_months`, ADR-012) діють автоматично: у `warned` (≥80 %) прогрів триває на звичайних моделях (як і зараз); у **`budget` (≥100 %) нові warm-задачі не запускаються** (лише ті, що дитина реально відкрила «на холодну», — і то економ-моделями, як і будь-яка інша генерація в цьому стані, US-11.5 КП-6); у **`hard_stop` (≥110 %) warm повністю зупинено**. Це не нова гілка бюджетної машини — прогрів просто **не має привілею** «завжди виконуватись», на відміну від `safety_moderator`.
- **Додатковий запобіжник саме для «пакетного» позначення багатьох тем одразу** (тато міг позначити одразу 10–20 тем предмета «поточними», D-65) — щоденний м'який ліміт витрат на прогрів `LIBRARY_WARM_DAILY_BUDGET_USD` (за замовчуванням **$5/день**, налаштовується): нова колонка `ai_calls.job_id` (nullable, FK → `jobs`) позначає виклики, зроблені **зсередини** warm-job; сума `cost_usd` за такими викликами за поточну добу (Київ) порівнюється з лімітом перед стартом кожного нового warm-job у `tick` — при перевищенні решта чекає наступної доби. Це не заміняє місячний ліміт $100 (ADR-012), а лише **згладжує** одноразовий сплеск від одного пакетного рішення тата в часі, щоб денне сповіщення про витрати (якщо тато дивиться в кабінет того ж дня) не показало несподіваний стрибок. **Відкрите питання до PO:** чи прийнятна сама ідея денного м'якого ліміту (і чи $5/день — розумне число), чи тато вважає за краще покладатися лише на місячні пороги 80/100/110 %.

**5. Що бачить дитина, якщо відкриває ще не прогріту тему.** Мінімум D-66 лишається чинним («Готуємо урок…», не порожній екран і не помилка). Уточнення для цього фіксу:
- Якщо для теми вже **є** активний warm-job (запущений раніше прогрівом чи іншою вкладкою) — дитина **не запускає другий** конвеєр (розд. 6 нижче), а підключається до того самого job і бачить його прогрес.
- Екран очікування показує **послідовний прогрес** замість статичного напису: job зберігає поточну стадію в `jobs.payload.stage` (`planning` → `generating` → `reviewing` → `revising` → `saving`), яку оновлює сам конвеєр після кожного кроку; клієнт опитує статус (той самий механізм, що вже показує прогрес індексації книги на «Мої книги», ADR-015) і показує людяний підпис («Продумую план уроку…» → «Пишу пояснення…» → «Перевіряю якість…»). Це не пришвидшує сам конвеєр, але зменшує відчуття «завислого» очікування (частина принципу P-H — «людяність» стосується і того, як система спілкується про власне очікування, не лише педагогіки).
- Орієнтовний час — статична підказка «зазвичай ≈ 1 хвилина» (з фактичного медіанного часу перших прогонів конвеєра, `docs/STATUS.md` після 10–15 реальних генерацій, як і калібрування рубрики рецензента, ADR-022) — без точного відліку секунд (непередбачувано через доопрацювання).

**6. Уникнення дублювання роботи (де-дуплікація).** `jobs.dedupe_key = 'library.warm:{topic_id}'` — **унікальний**; перед створенням задачі (і з боку прогріву при позначенні «поточної», і з боку дитячого «холодного» відкриття теми) сервер:
1. Перевіряє `library_items.status = 'active'` для теми — якщо вже є, конвеєр не запускається (звичайний кеш-путь бібліотеки, ADR-014, без змін).
2. Якщо немає — шукає **існуючу** задачу з тим самим `dedupe_key` у статусі `queued`/`running` — якщо є, **не** створює нову, а дитяча сесія (чи `parent`-прогрів) лише підписується на її прогрес (крок 5 вище).
3. Лише якщо немає ні активного блоку, ні задачі в черзі — створює нову задачу; дитяче «холодне» відкриття теми створює задачу з **негайним запуском** (`run_after = now()` + одразу `kickJobs()` у тому самому запиті, той самий патерн, що вже є для індексації, ADR-015, примітка S1) — тобто «холодний» шлях **не чекає** щохвилинного `pg_cron`, а прогрів за сигналом тата може почекати кілька секунд (не потребує негайності).

**Наслідки для реалізації.** Немає нових таблиць; нові поля — `jobs.payload.stage` (jsonb, вже існуюче поле `payload`, нового стовпця не потрібно) і одна нова колонка `ai_calls.job_id` (nullable, для щоденного ліміту прогріву, п. 4). Немає нової ролі маршрутизатора — прогрів використовує ті самі ролі `lesson_planning`/`lesson_generation`/`lesson_review`, що й «холодний» шлях (розд. 7.3), тому таблиця `model_routes` не змінюється. Деталі, вартість і альтернативи — ADR-023, `docs/03-resources-and-costs.md` розд. 3.2б.

### 7.7b. Два бюджети швидкості — наскрізний принцип (D-77, ADR-023)

> **Простими словами.** PO загострив 7.7a реальним прикладом: натиснув майже всі теми — усі «висять». Але ще важливіше нове уточнення PO: **та сама** проблема («важкий ланцюжок кількох провайдерів у шляху дитини») не має права повторитись там, де дитина вже *всередині* уроку й чекає на живу реакцію (пояснення помилки, ще одна вправа на закріплення, «поясни задачу №N»). PO порівнює: жива розмова з ШІ (як з Claude напряму) — майже без пауз; **генерація тексту вручну не має коштувати стільки ж часу, скільки рендер відео**. Тому систему свідомо ділимо на два явно різні «бюджети швидкості» з різними правилами, а не намагаємось прискорити один універсальний конвеєр для всього.

**Шар 1 — повільний, якісний, ОДНОРАЗОВИЙ (уже описано в 7.7/7.7a).** Повний конвеєр `lesson_planning → lesson_generation → lesson_review (інший провайдер) → ≤ 2 доопрацювання` — **застосовується лише** для створення офіційного, кешованого, версійованого контенту бібліотеки (`library_items.kind = block/review_set/diagnostic_bank`, ADR-014/022). Дитина цей шлях **синхронно майже ніколи не бачить** — його ховає прогрів (7.7a) і кеш; типова тривалість ≈ 1 хв — прийнятно, бо відбувається наперед, не в момент очікування дитини.

**Шар 2 — швидкий, ЖИВИЙ, ІНТЕРАКТИВНИЙ (новий, наскрізний, D-77).** Усе, що відбувається в реальному часі, поки дитина чекає прямо зараз:

| Що | Приклад | Роль (модель) | Правило |
|---|---|---|---|
| Оцінка відповіді | US-6.3, D-75/BUG-019…022 | `answer_evaluation` (Sonnet 5) — **уже так** | Один виклик, один провайдер; здебільшого детерміновано (правильність результату — без ШІ, де можливо, D-75) |
| Пояснення помилки й **ще одна вправа на закріплення** | **US-6.15, D-74** — найважливіше застосування цього принципу | **новий** `step_reinforcement` (Sonnet 5, один виклик, **без** `lesson_planning`/`lesson_review` у шляху) | Одна швидка генерація «схожого завдання на той самий навик» у контексті кроку (тема, еталон, тип помилки з `answer_evaluation`) — **не** повний конвеєр; ціль ≤ 5–8 с (той самий порядок латентності, що вже заявлений для `answer_evaluation`, розд. 7.3) |
| Чат «ШІ-друг» | US-8.x | `friend_chat` (Sonnet 5) — **уже так** | Без змін |
| «Поясни задачу №N» з підручника | **US-8.7, D-72/73, посилено D-78** | **retrieval + один швидкий виклик** — **не** конвеєр узагалі | (1) **Retrieval, без ШІ:** пошук уже проіндексованого тексту задачі за номером вправи в `chunks`/`material_sections` (US-2.8, S1/S1b — дані вже готові, PO прямо на це вказує в D-78: «ми розпізнали тексти підручників, ми витягнули індексацію, ми маємо структуру»); (2) **один** виклик `tutor_chat`-подібної ролі (Sonnet 5) з цим фрагментом як контекстом — пояснення методу → дати спробувати → перевірити (сократично, D-73), кожна репліка — окрема швидка відповідь чату, як «ШІ-друг». **Немає** ні `lesson_planning`, ні `lesson_generation`, ні синхронного `lesson_review` — цей сценарій архітектурно **ближчий до «ШІ-друга»** (легкий чат поверх retrieval-контексту з підручника), а не до генерації бібліотеки уроків, яку прогріваємо за D-76 |
| Голосовий діалог | розд. 7.4 | `voice_agent` — **уже так** | Без змін (уже задовольняє ≤ 1,5 с) |

**Уточнення PO (D-78, аналогія з онлайн-курсом).** PO прямо порівнює продукт з купівлею онлайн-курсу: контент там не генерується на льоту — відкривається одразу, єдине можливе очікування — підвантаження **відео** (якого в нас немає, лише текст і легкі інтерактивні кроки). Для сценарію «задача №N» **уся потрібна інформація вже лежить у вже проіндексованих даних** (S1/S1b: текст задачі, сторінка, тема — `chunks`/`material_sections`/`topics`), тож там, на відміну від Шару 1 (де реально потрібно, щоб ШІ **створив** новий методичний матеріал), очікування генерації в принципі не повинно виникати — це задача пошуку (retrieval, без ШІ) + одна швидка відповідь ШІ поверх уже знайденого тексту, а не створення нового контенту.

**Тверде архітектурне правило (перевіряється код-рев'ю/`qa-tester`, не лише угода):** **жоден маршрут, що виконується в межах живого запиту дитини (кроку уроку, чату, голосу), не викликає `lesson_review` синхронно** і не чекає на відповідь іншого провайдера як обов'язковий крок перед показом результату дитині. Якщо для якості закріплювальних завдань колись знадобиться незалежна рецензія — вона виконується **асинхронно, заднім числом**, у фоновій задачі, **після** того, як дитина вже отримала швидку відповідь, і лише покращує **майбутнє** (див. нижче), а не блокує **зараз**.

**Куди дівається якість, якщо рецензії немає в моменті.** Компроміс свідомий (не «менше якості без причини»): швидка вправа на закріплення (`step_reinforcement`) — **одноразова й персональна** (під конкретну помилку конкретної дитини зараз), а не кандидат у спільну бібліотеку сама по собі. Щоб не втратити довгострокову якість і принцип P-R (економія через перевикористання):
- Результат `step_reinforcement` **не зберігається** як `library_items` (не проходить кеш/рецензію) — це ефемерний контент сесії (`step_attempts`/лог кроку), як і сьогоднішні альтернативні пояснення до першої версії ADR-014;
- Окрема **фонова** задача (`jobs` тип `library.harvest_reinforcement`, низький пріоритет, не блокує нічого) раз на день бере зразок реально використаних `step_reinforcement`-відповідей (без ПД, розд. 5.4) і **лише для показових/повторюваних випадків** (той самий тип помилки в тій самій темі трапляється часто) пропускає найкращий приклад через повний конвеєр `lesson_generation`→`lesson_review` **асинхронно**, щоб перетворити його на офіційний `library_items.kind='alt_explanation'` — тобто **наступного разу** для схожої помилки система вже має готове, рецензоване пояснення (кеш), а не лише швидку одноразову генерацію. Це і є «асинхронна рецензія заднім числом для покращення бібліотеки на майбутнє», про яку прямо просив PO.
- Безпека **не** послаблюється в жодному разі: `safety_moderator` (шар 2, розд. 7.3) лишається синхронним і обов'язковим для будь-якої репліки дитині незалежно від шару швидкості (P-S, NFR-SAFE-13) — це вже дешевий і швидкий виклик, тому не суперечить бюджету швидкості.

**Наслідки для `model_routes` (розд. 6.1).** Додається новий рядок ролі `step_reinforcement` (основна — Claude Sonnet 5, резерв — GPT-5.6 Terra, економ — Gemini 3.8 Flash; **без** поля ескалації на важчий ланцюжок — ескалація тут означає лише сильнішу модель **того самого** одноразового виклику, ADR-005, ніколи не «підключити повний конвеєр»). Вартість — на рівні `answer_evaluation`/`tutor_chat` (розд. 3.1, П-5/П-7: одиниці й десятки центів на місяць, не десятки центів на виклик) — оцінка додається в `docs/03-resources-and-costs.md` розд. 3.2в.

**Межа мандату цього документа.** Технічна реалізація самого циклу «пояснити → закріпити → рухатись далі» (US-6.15) — предмет окремого зрізу (S27, `product-manager`/наступний прохід `architect`+`developer`); цей розділ фіксує **обов'язковий архітектурний принцип**, за яким та реалізація має проєктуватися (Шар 2, не Шар 1), а не саму реалізацію кроку за кроком.

**Відкрите питання до PO — залишок «холодного» шляху (7.7a) і 30-секундна ціль.** Прогрів (7.7a) прибирає більшість випадків, коли дитина взагалі торкається Шару 1 напряму, але **не гарантує** математично ≤ 30 с для теми, яку прогріти не встигли (щойно позначена, черга зайнята, бюджет у режимі `budget`) — повний конвеєр з можливим доопрацюванням реально займає ≈ 1 хв, іноді більше. Дві чесні опції для такого рідкісного залишкового випадку — обрати повинен PO, бо це компроміс саме цінності «педагогічна якість перед показом» (D-55):
1. **Прийняти ≈ 60–90 с у рідкісному «холодному» випадку**, компенсуючи це чесним прогресом («Продумую план… → Пишу пояснення… → Перевіряю якість…», 7.7a п. 5) замість статичного спінера — і покладатись на прогрів (7.7a), щоб такий випадок ставався рідко; **або**
2. Додати **тимчасовий «швидкий чернетковий» перший крок** для теми, яку справді не встигли прогріти: один швидкий виклик (без `lesson_planning`/`lesson_review`, лише синхронна модерація безпеки) показує дитині перший слайд протягом ~15–20 с, поки повний якісний конвеєр доганяє у фоні й **замінює** чернетку офіційним, рецензованим блоком для всіх майбутніх відкриттів цієї теми. Це технічно дає «~30 с і щось на екрані», але означає, що **у цьому одному, рідкісному випадку** дитина певний час бачить контент, який ще не пройшов незалежну рецензію якості (ризик методичної/фактичної помилки зменшений — синхронна модерація безпеки все одно є — але не нульовий) — пряме відхилення від правила «лише `active`/рецензований контент — дитині» (ADR-022, розд. 7.7 п. 5) для цього вузького випадку.
Рекомендація архітектора: **варіант 1** за замовчуванням (простіше, не порушує принцип якості з D-55 навіть один раз); переглянути на варіант 2 лише якщо після впровадження прогріву (7.7a) телеметрія покаже, що «холодні» відкриття все ще трапляються часто на практиці.
...
> - **Багаторічність (ADR-021, розд. 14.4):** таблиця **`academic_years`** (`label`, `grade`, `starts_on`, `ends_on`, `status`); **`academic_year_id`** (default — активний рік) у `lesson_sessions`, `chats`, `diagnostic_runs`, `homework`, `reports`, `points_ledger`; **`grade`** + **`curriculum_version`** у `topics`, `materials`, `library_items`; `materials.edition_year`; `topics.supersedes_topic_id`; `subjects.config.continues_subject_id`; `profile_statements.last_confirmed_at`; `embedding_model` у `chunks` і `messages`.
> - Для стислості `family_id` і поля багаторічності в таблицях нижче не повторюються.
...
| Таблиця | Ключові поля | Примітки |
|---|---|---|
| `library_items` | `topic_id`, `kind` (`block`/`review_set`/`diagnostic_bank`/`alt_explanation`), `title`, `version`, `status` (`draft`/`active`/`needs_review`/`superseded`), `replaced_reason`, `interest_tag`, `model`, `prompt_version`, `source_refs`, **`pedagogy jsonb`** (методичний паспорт: `learning_goal`, `misconceptions[]`, `techniques[]`, `review` — розд. 7.7, ADR-022), **`child_feedback jsonb`** (агрегат «цікаво/корисно» від дитини, US-19.4), `created_at` | «Перегенерувати» створює нову версію, стара — `superseded` (US-19.4 КП-2). Оркестратор при плануванні уроку бере **лише** `status='active'` (пройшов рецензію, розд. 7.7 п. 5); `needs_review` дитині не показується. |
| `library_item_reviews` | `library_item_id`, `iteration` (1/2), `reviewer_role` (`lesson_review`), `provider`, `model`, `verdict` (`approved`/`revise`/`rejected`), `scores jsonb` (бали по критеріях рубрики), `notes`, `created_at` | Історія рецензій блоку (ADR-022); остання — джерело `library_items.pedagogy.review`; аудит для тата й `qa-tester`. |
...
sequenceDiagram
  ...
  API->>L: є активний (`status=active`) блок у бібліотеці?
  alt немає
    API->>R: lesson_planning (Opus 5.5) + фрагменти підручника + профіль
    R-->>API: методичний план (мета, міскончепції, прийоми)
    API->>R: lesson_generation (Opus 5.5) + план + фрагменти підручника
    R-->>API: блок (JSON)
    API->>R: lesson_review (GPT-5.6 Sol, інший провайдер) — рубрика
    R-->>API: вердикт (approved / revise / rejected)
    opt revise, ≤ 2 ітерації
      API->>R: lesson_generation (доопрацювання за зауваженнями)
      API->>R: lesson_review (повторно)
    end
    alt approved
      API->>L: зберегти (status=active, pedagogy-паспорт); job tts (озвучка пояснень)
    else needs_review (не approved після 2 ітерацій)
      API->>L: зберегти (status=needs_review, дитині не показується)
      API->>API: подія тату «потрібна ручна перевірка блоку»
      API-->>UI: показати інший наявний блок теми або повідомлення
    end
  end
...
### 10.3. Завантаження книги з комп'ютера в «Мої книги» (US-2.7 КП-5/PM-20, ADR-024)

> **Питання PO:** чи можна дати татові завантажити файл підручника прямо з браузера кабінету, а застосунок сам покладе його на Google Drive і пропустить через наявний конвеєр індексації (9.1), як зараз — файли, покладені вручну. **Відповідь: так, і це не вимагає ширшого доступу до Drive тата** — варто лише спрямувати завантажений файл у **нову**, створену самим застосунком папку («ШІ-Репетитор — Мої книги»), а не в наявну папку матеріалів, яку тато й так наповнює вручну. Деталі, альтернативи й наслідки — ADR-024.

**Чому не пряме записування в наявну папку матеріалів.** Наявна папка `GOOGLE_DRIVE_FOLDER_ID` доступна сервісному акаунту тому, що тато сам вручну дав йому доступ «Читач» через звичайний Drive UI (поза OAuth-згодою застосунку) — жодна `drive.file`-згода тата **не покриває** цю чужу для застосунку папку. Два способи це виправити:
1. Тато один раз обирає саму папку матеріалів через **Google Picker** — технічно дає `drive.file`-токену доступ до цієї конкретної папки. Це саме та альтернатива, яку ADR-003 вже відхилив як незручну (там — для кожного файлу; тут — хоч і одноразово для папки, все одно окремий незвичний UI-крок «оберіть ту саму папку в іншому вікні», що плутає «навіщо вибирати папку, яку я і так уже відкрив у Drive»).
2. **Обрано:** застосунок створює **свою власну** папку (як уже заплановано для архіву, ADR-013) — до неї `drive.file`-доступ є **автоматично**, без Picker, бо `drive.file` завжди покриває об'єкти, створені самим застосунком.

**Рішення одним реченням.** Один спільний OAuth `drive.file`-конектор («Підключити Google Drive» — той самий, що вже запланований для архіву S17) створює **дві** приватні папки в «Мій диск» тата — «ШІ-Репетитор — Архів» (ADR-013) і **нову** «ШІ-Репетитор — Мої книги»; одразу після створення другої застосунок тим самим токеном (тато — власник папки, може нею ділитися) викликає `permissions.create()`, надаючи сервісному акаунту `ai-tutor-drive-reader` роль «Читач» на цю нову папку — без ручного кроку «Поділитися» від тата. Далі папка `GOOGLE_DRIVE_UPLOADS_FOLDER_ID` для сервісного акаунта нічим не відрізняється від `GOOGLE_DRIVE_FOLDER_ID`: той самий конвеєр 9.1 (список файлів → перевірка публічності → `ingest.extract` → `ingest.embed` → `ingest.structure`) сканує **обидві** папки — жодних змін у самому конвеєрі й у `materials`/`chunks`.

**Тригер індексації швидший, ніж для наявної папки.** Оскільки сервер сам щойно створив файл через Drive API, він уже знає `drive_file_id`, `name`, `mime`, `modified_time` з відповіді `files.create` — не потрібно чекати наступного «Перевірити папку» чи щохвилинного `pg_cron`: рядок `materials` вставляється й `ingest.extract` ставиться в чергу з негайним запуском (`kickJobs()`, той самий патерн, що вже є для «холодного» відкриття теми, ADR-015/023) у тому самому запиті завантаження.

**Обмеження розміру браузерного завантаження.** `MAX_FILE_BYTES = 150 МБ` (`app/src/server/drive/google.ts`) сьогодні захищає лише **завантаження з Drive під час індексації** (BUG-003) — це не той самий шлях, що вивантаження браузер → сервер → Drive, і напряму на нього не переноситься. Для **вивантаження** з браузера в serverless-функцію Vercel діють інші, суворіші практичні обмеження (розмір тіла запиту й тривалість виконання; подвійний транзит клієнт→Vercel→Drive для 150 МБ на типовому домашньому каналі ризикує таймаутом і витратою пам'яті функції). Тому:
- практичний ліміт **прямого завантаження з кабінету — ≈ 50 МБ** (типовий текстовий PDF/EPUB укладається з великим запасом; окреме поле конфігурації, не перевикористовує `MAX_FILE_BYTES`);
- ендпойнт вивантаження **стрімить** тіло запиту напряму в Drive resumable-upload, рахуючи байти на льоту (той самий інженерний патерн, що вже виправив BUG-003 для протилежного напрямку — не буферизувати весь файл у пам'яті функції) — і відмовляє одразу після перевищення ліміту, не чекаючи повного прийому;
- файли **більші за 50 МБ** (скани підручників у високій роздільності) — і далі через наявний шлях «покласти в папку Drive вручну» (без цього ліміту, лише `MAX_FILE_BYTES=150 МБ` на боці індексації) — панель «Додати книгу» лишає обидва шляхи: новий «Завантажити файл» як зручний варіант за замовчуванням і наявний «Відкрити папку в Google Drive» для великих/масових випадків;
- сервер перевіряє MIME (лише PDF/EPUB) і що цільова папка — саме наша `GOOGLE_DRIVE_UPLOADS_FOLDER_ID` (захист від помилкового запису не туди), а також публічність нової папки — тим самим механізмом, що вже є для архіву (10.2, будь-який дозвіл `anyone` → застереження/блокування).

**Сховище: лише Drive, без дублювання в Supabase Storage.** На відміну від D-67/ADR-013 (де генерований медіаконтент і чати живуть **насамперед** у Supabase Storage, а Drive — лише щомісячна архів-копія для довговічності), для книг-джерел копія в Storage не додається: увесь конвеєр S1/S1b і так вважає Drive канонічним сховищем підручника (`materials.drive_file_id`, `downloadFile` читає файл із Drive при кожному кроці індексації) — файли, завантажені з браузера, нічим не відрізняються від файлів, покладених татом вручну, і не мають ставати винятком із цього правила. Дублювання додало б зайве сховище й ризик розбіжності версій без жодної вигоди (книга — не персональні дані дитини, для неї не потрібен швидкий підписаний доступ з чату, як для фото/аудіо).

**Приватність і межа доступу (не розширюємо доступ застосунку).** `drive.file` лишається **не чутливим** скоупом (перевірка Google не потрібна, розд. 10.1); токен, навіть якщо його буде скомпрометовано, дає доступ **лише** до двох папок, які застосунок сам створив (архів і завантажені книги) — не до решти «Мій диск» тата. Підручник — не персональні дані дитини (нижчий ризик приватності, ніж чат чи медіа дитини), тому додаткових вимог NFR-PRIV тут немає, окрім вже наявних (перевірка публічності, токен у Vault).

**Наскільки це великий шматок роботи й чи потрібна нова фаза погодження акаунтів.** Google Cloud OAuth consent screen для `drive.file` **вже налаштований** у чеклісті (`docs/03-resources-and-costs.md`, розд. 1.5, крок 4) — заново нічого узгоджувати в консолі Google не потрібно. Новою є лише **дія тата в самому застосунку**: один клік «Підключити Google Drive» (той самий екран, що вже описаний для архіву, `docs/03` розд. 1.6.6) — без окремої повторної авторизації для книг. Це середній за розміром зріз розробки (≈ 4–5 днів): спільний OAuth-конектор (частина роботи, яка й так потрібна для S17, лише виконується раніше), автоstore/автоshare нової папки, стрімінговий upload-ендпойнт з лімітом і прогресом, кнопка «Завантажити файл» в «Додати книгу» (`AddBookPanel`), тести. Рекомендація — окремий беклог-зріз (не всередині S27/S28), запланувати `product-manager`.
...
| **Інтерактивні й анімовані уроки (стратегічний напрям; R2, частково MVP — прийнято 2026-09-25)** | Формат кроку `visual.component` + `v` + `props`, тип кроку `interactive`, реєстр компонентів із Zod-валідацією й детермінованою оцінкою; 3 базові компоненти (`number_line`, `drag_sort` з варіантом «шкала часу», `step_reveal`) — розд. 14.3, ADR-020 | Нові компоненти (`fraction_bar`, `timeline`, `map`, `diagram_hotspots`, `animated_scene`) — папка + рядок у `subjects.config.allowed_components`, без зміни схеми |
...
| **Комерціалізація (можливо) / ШІ-онлайн-школа** | `families`, `family_id` + RLS за сім'єю, `owner_family_id` для контенту, облік витрат по сім'ї, інтеграції через `getFamilyIntegration()`; вимір «клас / програма» в контенті (розд. 14.4) — основа спільної бібліотеки за класами | Оплати, реєстрація сімей, юридичне, платні тарифи хостингу — окрема фаза |
| **Багаторічність 6→11 клас (горизонт 7 років)** | `academic_years`, `grade` і `curriculum_version` у темах і матеріалах, `academic_year_id` у сесіях/чатах/звітах, наступник теми, щомісячний експорт даних на Drive, `embedding_model` — розд. 14.4, ADR-021 | Майстер «Перехід у наступний клас» (до вересня 2027), кнопка «Експортувати все», перегляд архіву років |
| **Професійні відеоуроки (пізня фаза, ідея без зобов'язань)** | Сценарій уроку вже структурований (кроки, кадри `step_reveal`, озвучка в `audio_assets`, `alignment`); фонові задачі `jobs`; маршрутизатор ролей — ADR-016 | Роль `video_generation`, задача `video.render`, `library_items.kind='video'`, тип витрат `video`; постачальник і ціни — не обрано, окреме рішення |
...
- **MVP — прийняте рішення PO 2026-09-25 («повний мінімум»):** формат + реєстр + `number_line`, `drag_sort`, `step_reveal` ≈ **4–5 днів розробника** (+ ≈ 0,5 дня дизайнера). «Шкала часу» для історії — варіант `drag_sort` (`props.variant = "timeline"`: події розставляються по порядку на шкалі); окремий компонент `timeline` — R2. Вартість генерації ≈ +$0,05–0,1 на новий урок (≈ +$1/міс у «Економі», враховано в кошторисі); повтори — $0.

### 14.4. Багаторічність: 6→11 клас, 7 років (ADR-021)

> **Простими словами.** Застосунок має «дорости» разом з донькою до 11 класу. Тому вже зараз кожна тема й підручник знають, **для якого вони класу й за якою програмою**, а кожен урок, чат і звіт — **до якого навчального року** належать. Коли настане 7 клас, старі дані не зникнуть і не змішаються з новими: карта знань і профіль переходять далі, минулий рік стає архівом. Крім того, раз на місяць повна копія даних у звичайних файлах лягає у вашу папку на Drive — дані не залежать від одного постачальника.

| Що закладаємо в MVP | Як |
|---|---|
| Навчальний рік як вимір | Таблиця `academic_years` (`label` «2026/27», `grade` 6, `starts_on`, `ends_on`, `status` `active`/`archived`); у MVP один рядок. `academic_year_id` (default — активний рік, тригер) у `lesson_sessions`, `chats`, `diagnostic_runs`, `homework`, `reports`, `points_ledger` |
| Клас і версія програми в контенті | `grade smallint` + `curriculum_version text` (напр., `NUS-2022`) у `topics`, `materials`, `library_items`; `materials.edition_year`. Предмети (`subjects`) — наскрізні між роками; поділ «Математика → Алгебра + Геометрія» у 7 класі = нові рядки `subjects` з `config.continues_subject_id` |
| Перенесення карти знань | `knowledge_state` прив'язана до теми (а тема — до класу), тому переходить між роками без копіювання; `topic_dependencies` можуть зв'язувати теми різних класів (алгебра 7 → дроби 6). Нова редакція підручника → нові теми з `topics.supersedes_topic_id` → рівень переноситься на наступника |
| Архів років | Дані минулих років лишаються в БД (тексти малі); UI за замовчуванням показує активний рік, фільтр «Рік» у пошуку чатів і звітах; бібліотека минулих класів — для повторення |
| Профіль «росте» | `profile_statements.last_confirmed_at`; твердження старші за рік — у щорічному переліку «Перевірте, чи це ще актуально» для тата (R2, логіка), поле — MVP |
| Довговічність і незалежність | Щомісячний архів (ADR-013) доповнюється **експортом даних сім'ї** у відкритих форматах (JSON + CSV + читабельний Markdown, без ембедингів) у `Архів/data-export/YYYY-MM/`; `embedding_model` у `chunks` і `messages` (переіндексація при зміні моделі); промпти, налаштування агента ElevenLabs і маршрути — у репозиторії/БД, не лише в консолях провайдерів |

- **Вартість закладання в MVP:** ≈ **1,5 дня розробника** (схема й тригер ≈ 0,5; експорт у щомісячному архіві ≈ 0,5; фільтр «Рік» і `grade` у індексації ≈ 0,5), **$0/міс** (експорт — кілька МБ на місяць на Drive).
- **Свідомо відкладено:** майстер «Перехід у наступний клас» (закрити рік, знімок карти знань, план повторення ключових тем після літа, підключити нові підручники) — **до вересня 2027**, ≈ 2–3 дні; кнопка «Експортувати все»; перегляд архіву років у кабінеті; кілька дітей і спільна бібліотека за класами (ADR-018).
...
| AR-R20 | За 7 років зміняться провайдери ШІ, ціни, тарифи хостингу, можливо — сам стек; постачальник може закритися | Довговічність | Дані — у Postgres (відкритий формат, переносний); щомісячний експорт на Drive; маршрутизатор моделей; `embedding_model` для переіндексації; щорічний перегляд стеку (ADR-021) |
| AR-R21 | Зміна програми НУШ / нові редакції підручників; поділ предметів у 7 класі | Карта знань | `curriculum_version`, `supersedes_topic_id`, `continues_subject_id`; ручна корекція татом (`manual_override`) |
| AR-R22 | Обсяг БД за 7 років (ембединги повідомлень ≈ 3 КБ кожне) | Тариф Supabase | Оцінка ≤ 1–2 ГБ за 7 років — у межах Pro (8 ГБ); за потреби — видалення ембедингів архівних років (текстовий пошук лишається) |
| AR-R23 | Педагогічний конвеєр (планування → генерація → рецензія) подовжує час підготовки **нового** блоку (кілька послідовних викликів, ≈ 10–20 с додатково) і додає ≈ +35 % до вартості нового уроку; рецензент може систематично бути занадто суворим/м'яким | UX очікування (лише для нових блоків, не повторів), вартість, якість фільтра | Попереднє завантаження наступного блоку (як і зараз, розд. 9.2) ховає затримку; ліміт 2 ітерацій запобігає нескінченному циклу; пороги рубрики й вибір моделі рецензента переглядаються після перших 10–15 рецензій (запис у `docs/STATUS.md`); паспорт і `needs_review`-подія дають тату контроль (ADR-022) |
...
| Рішення PO 2026-09-25: інтерактивні й анімовані уроки (**повний мінімум у MVP прийнято**) | розд. 0, 8.2 (`library_steps`), 14, 14.3; ADR-020; `docs/03` розд. 3.9а, 5 |
| Рішення PO 2026-09-25: багаторічність 6→11 клас, можливе зростання в ШІ-онлайн-школу | розд. 0, 8, 14, 14.4, 16 (AR-R20…R22); ADR-021; `docs/03` розд. 5 |
| Рішення PO 2026-09-25: відеоуроки — пізня фаза, лише точка розширення | розд. 14; ADR-016 |
| Рішення PO D-55 (2026-09-26): педагогічний конвеєр генерації уроку (планування → сценарій → незалежна рецензія іншим провайдером → доопрацювання → методичний паспорт); книги з Drive — лише база програми | розд. 0 (варто оновити на демо), 7.3, 7.7, 8.2, 9.2, 16 (AR-R23); ADR-022; примітки в ADR-005, ADR-014; `docs/03` розд. 3.2, 3.5 |
| Доповнення PO 2026-09-26: «не вирішувати задачу в лоб» (гачок, прогрес, естетика, тон); переоцінка голосу (Claude/OpenAI/Gemini API vs ElevenLabs) | розд. 7.4, 7.7; ADR-006 (статус «пропозиція»), ADR-022; `docs/03` розд. 6 |
| Рішення PO D-76/D-77/D-78 (2026-09-26): фонове прогрівання бібліотеки за сигналом D-65 (елевація D-66); два бюджети швидкості (Шар 1 — одноразова генерація бібліотеки, Шар 2 — швидкий живий інтерактив, зокрема майбутнє US-6.15/S27); retrieval, а не генерація, для «поясни задачу №N» (US-8.7/US-2.8) | розд. 4 (`jobs`), 6.1–6.2, 7.3 (`step_reinforcement`), 7.7a, 7.7b; ADR-023; `docs/03` розд. 3.2б, 3.2в |
